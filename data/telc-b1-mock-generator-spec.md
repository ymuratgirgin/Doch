...
- These names exist ONLY to cast a voice for the script and must never appear in the richtig/falsch statements. The real exam always refers to speakers neutrally there — e.g. "Die Sprecherin muss im Haushalt fast alles alleine machen." (Teil 1), "Der Journalist unterhält sich mit einer Vertreterin des Sportvereins." (Teil 2) — never "Frau Ahmadi ..." or "Herr Bauer ...". Write every statement using "der Sprecher"/"die Sprecherin", "der Journalist"/"die Interviewpartnerin", or the person's role/situation, exactly as if the invented name in the script didn't exist.
- Mark pauses with a line break and "(Pause)" between segments; the learner inserts silence or simply pauses playback there.
- Keep spoken-language features to real, fully-spelled words (na ja, tja, also, contractions) that TTS pronounces naturally. **Never write hesitation interjections like "ähm", "äh", "hm"** — a human reader voices these as a natural pause, but text-to-speech reads the letters as a mispronounced word instead. If a script does end up with one anyway, the app strips it before synthesis as a safety net, but don't rely on that — write around it instead.
...
- **Teil 1 (41–45):** 5 short monologues (40–80 words each) by different everyday speakers, each independently answering the same shared survey question (e.g. an intro line like "Wir beschäftigen uns heute mit der Frage 'Gleichberechtigung im Haushalt' ... Hören Sie dazu einige Stellungnahmen:" followed by 5 people's individual answers) — the speakers don't address or respond to each other, only the shared topic. Natural spoken features: na ja, tja, contractions (see the interjection rule above). 1 richtig/falsch statement per speaker. Header must say the texts are heard **only once** and give 30 seconds to pre-read.
- **Teil 2 (46–55):** one interview (~450–600 words), radio style: journalist + expert/interesting person (club anniversary, unusual job, local project). 10 richtig/falsch statements in interview order. Mix clearly-stated facts with statements that contradict a detail (numbers, who does what, since when). Heard twice; 60 seconds pre-reading. Only the journalist ever says the guest's name, once, while introducing them at the start (e.g. "Ich begrüße heute Frau Seiffert. Guten Tag!") — the guest replies naturally ("Guten Tag!") and is never scripted to state their own name later, since a real person doesn't reintroduce themselves mid-conversation.
